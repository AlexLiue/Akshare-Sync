...
def exec_sync(args):
    ...
    if start_date < end_date:
        logger.info(
            f"Execute Sync stock_zh_a_hist_daily_qfq  trade_code[{trade_code}] trade_name[{trade_name}] from [{start_date}] to [{end_date}]"
        )
        df = stock_zh_a_hist(
            symbol=trade_code,
            period="daily",
        ...
        if not df.empty:
            ...
            if (
                ...
                logger.info(
                    f"Execute Sync stock_zh_a_hist_daily_qfq trade_code[{trade_code}]"
                    + f" Write[{df.shape[0]}] Records"
                )
            else:
                clean_sql = f"DELETE FROM STOCK_ZH_A_HIST_DAILY_QFQ WHERE \"股票代码\"='{trade_code}'"
                logger.info(
                    f"Execute Sync stock_zh_a_hist_daily_qfq, Detect QFQ data updated, Clean History Data With SQL [{clean_sql}], Recall Sync"
                )
                exec_sql(clean_sql)
        ...
        logger.info(
            f"Execute Sync stock_zh_a_hist_daily_qfq  trade_code[{trade_code}] trade_name[{trade_name}] from [{start_date}] to [{end_date}], Skip Sync ... "
        )
...
def sync(drop_exist=False, enable_proxy=False, max_workers=5):
    ...
    try:
        ...
        update_sync_log_date(
            "stock_zh_a_hist", "stock_zh_a_hist_daily_qfq", f"{str(end_date)}"
        )
