...
def exec_sync(args):
    ...
    start_date = (
            datetime.datetime.strptime(last_sync_date, "%Y%m%d") + relativedelta(weeks=1)
    ).strftime("%Y%m%d")

    if start_date < end_date:
        logger.info(
            f"Execute Sync stock_zh_a_hist_weekly_hfq  trade_code[{trade_code}] trade_name[{trade_name}] from [{start_date}] to [{end_date}]"
        )
        df = stock_zh_a_hist(
            symbol=trade_code,
            period="weekly",
        ...
        if not df.empty:
            ...
            logger.info(
                f"Execute Sync stock_zh_a_hist_weekly_hfq trade_code[{trade_code}]"
                + f" Write[{df.shape[0]}] Records"
            )
    else:
        logger.info(
            f"Execute Sync stock_zh_a_hist_weekly_hfq  trade_code[{trade_code}] trade_name[{trade_name}] from [{start_date}] to [{end_date}], Skip Sync ... "
        )
...
def sync(drop_exist=False, enable_proxy=False, max_workers=5):
    ...
    try:
        ...
        update_sync_log_date(
            "stock_zh_a_hist", "stock_zh_a_hist_weekly_hfq", f"{str(end_date)}"
        )
