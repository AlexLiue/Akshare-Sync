...
def exec_sync(args):
    ...
    if start_date <= end_date:
        logger.info(
            f"Execute Sync stock_zh_a_hist_monthly_hfq  trade_code[{trade_code}] trade_name[{trade_name}] from [{start_date}] to [{end_date}]"
        )
        df = stock_zh_a_hist(
            symbol=trade_code,
            period="monthly",
        ...
        if not df.empty:
            ...
            logger.info(
                f"Execute Sync stock_zh_a_hist_monthly_hfq trade_code[{trade_code}]"
                + f" Write[{df.shape[0]}] Records"
            )
    else:
        logger.info(
            f"Execute Sync stock_zh_a_hist_monthly_hfq  trade_code[{trade_code}] trade_name[{trade_name}] from [{start_date}] to [{end_date}], Skip Sync ... "
        )
...
def sync(drop_exist=False, enable_proxy=False, max_workers=5):
    ...
    try:
        ...
        update_sync_log_date(
            "stock_zh_a_hist", "stock_zh_a_hist_monthly_hfq", f"{str(end_date)}"
        )

    except Exception:
        logger.error(f"Table [stock_zh_a_hist_monthly_hfq] Sync  Failed", exc_info=True)
        update_sync_log_state_to_failed(
            "stock_zh_a_hist", "stock_zh_a_hist_monthly_hfq"
        )
