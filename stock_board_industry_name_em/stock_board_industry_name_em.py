...
def query_last_sync_date(engine, logger):
    query_start_date = f'SELECT NVL(MAX("日期"), 19700101) as max_date FROM STOCK_BOARD_INDUSTRY_NAME_EM'
...
def sync(drop_exist=False, enable_proxy=False):
    ...
    try:
        ...
        if start_date < end_date:
            ...
            if not df.empty:
                # 清理历史数据
                clean_sql = f"TRUNCATE TABLE STOCK_BOARD_INDUSTRY_NAME_EM"
                ...
                if not df.empty:
                    ...
                    logger.info(
                        f"Write [{df.shape[0]}] records into table [stock_board_industry_name_em] with [{engine.engine}]"
                    )
                    update_sync_log_date(
                        "stock_board_industry_name_em",
                        "stock_board_industry_name_em",
            ...
            logger.info("Table [stock_board_industry_name_em] Early Synced, Skip ...")
    except Exception:
        logger.error(f"Table [stock_board_industry_name_em] Sync Failed", exc_info=True)
        update_sync_log_state_to_failed(
